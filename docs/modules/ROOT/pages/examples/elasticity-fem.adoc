...
\begin{align}
    ...
    \int_{\Omega} \rho u_{tt} \cdot v \, dx + \int_{\Omega} \sigma : \epsilon(v) \, dx &= \int_{\Omega} f \cdot v \, dx + \int_{\Gamma} g \cdot v \, ds
\end{align}
++++

where stem:[\sigma : \epsilon (v)] denotes the maximal contraction of the tensors stem:[\sigma] and stem:[\epsilon(v)], equal to stem:[\sum_{i, j} \sigma_{ij} \epsilon(v)_{ij}]. Proof:
...
\sigma(\mathbf{u}) : \epsilon(\mathbf{v}) &= \left( (\lambda + 2 \mu ) u_{1i} + \lambda u_{2j} \right) v_{1i} \\
&+ \left( \mu (u_{1j} + u_{2i}) \right) \cdot \frac{1}{2}(v_{1j} + v_{2i}) \\
&+ \left( \lambda u_{1i} + (\lambda + 2 \mu ) u_{2j} \right) v_{2j} \\
&=  \left( (\lambda + 2 \mu )u_{1i} + \lambda u_{2j} \right) v_{1i} + \left( \mu (u_{1j} + u_{2i}) \right) v_{2i} \\
&+ \left( \lambda u_{1i} + (\lambda + 2 \mu ) u_{2j} \right) v_{2j} \\
\sigma(\mathbf{u}) : \nabla \mathbf{v} &= \sigma(\mathbf{u}) : \epsilon(\mathbf{v})
\end{align}
++++
...
++++
\begin{align}
\sigma(\mathbf{u}) : \epsilon(\mathbf{v}) &= \lambda(\nabla \mathbf{u})I : \epsilon(\mathbf{v}) + 2\mu\epsilon(\mathbf{u}) : \epsilon(\mathbf{v}) \\
&= \lambda(\nabla \mathbf{u})(\nabla \mathbf{v}) + 2\mu\epsilon(\mathbf{u}) : \epsilon(\mathbf{v})
\end{align}
++++
...
++++
\begin{align}
    a(u, v) &= \int_{\Omega} \rho u_{tt} \cdot v \, dx + \int_{\Omega} \sigma : \epsilon(v) \, dx \\
    a(u, v) &= \int_{\Omega} \rho u_{tt} \cdot v \, dx + \int_{\Omega} \lambda(\nabla \mathbf{u})(\nabla \mathbf{v}) + 2\mu\epsilon(\mathbf{u}) : \epsilon(\mathbf{v}) \, dx \\
    l(v) &= \int_{\Omega} f \cdot v \, dx + \int_{\Gamma} \sigma \cdot n \cdot v \, ds
\end{align}
++++
...
\begin{align}
    ...
    \int_\Omega \partial_t^2 u^0 \cdot v \, dx + \int_\Omega \sigma(u_0) : \epsilon(v) \, dx &= \int_\Omega f_0 \cdot v \, dx + \int_\Gamma g_0 \cdot v \, ds
\end{align}
++++
...
++++
\begin{align}
    \int_\Omega \partial_t^2 u^0 \cdot v \, dx &= \int_\Omega f_0 \cdot v \, dx - \int_\Omega \sigma(u_0) : \epsilon(v) \, dx + \int_\Gamma g_0 \cdot v \, ds \\
    \int_\Omega \partial_t^2 u^0 \cdot v \, dx &= \int_\Omega f_0 \cdot v \, dx - \int_\Omega \lambda(\nabla u_0)(\nabla v) + 2\mu\epsilon(u_0) : \epsilon(v) \, dx + \int_\Gamma g_0 \cdot v \, ds
\end{align}
++++
...
\begin{align}
    ...
    &= \frac{\Delta t^2}{2 \rho} \left( \int_\Omega f_0 \cdot v \, dx - \int_\Omega \lambda(\nabla u_0)(\nabla v) + 2\mu\epsilon(u_0) : \epsilon(v) \, dx + \int_\Gamma g_0 \cdot v \, ds \right) \\
    &= \frac{\Delta t^2}{2 \rho} \left( \int_\Omega f_0 \cdot v \, dx - \int_\Omega \lambda(\nabla u_0)(\nabla v) + 2\mu \mathbf{Tr} \left( \epsilon(u_0) \epsilon(v)^T \right) \, dx + \int_\Gamma g_0 \cdot v \, ds \right)
\end{align}
++++
...
++++
\begin{align}
    \int_\Omega \frac{u_{n+1} - 2 u_n + u_{n-1}}{\Delta t^2} \cdot v \, dx + \int_\Omega \sigma(u_n) : \epsilon(v) \, dx - \int_\Gamma g \cdot v \, ds &= \int_\Omega f_n \cdot v \, dx
\end{align}
++++
...
++++
\begin{align}
    \int_\Omega u_{n+1} \cdot v \, dx &= \frac{\Delta t^2}{\rho} \left( \int_\Omega f \cdot v \, dx - \int_\Omega \sigma(u_n) : \epsilon(v) \, dx + \int_\Gamma \sigma(u_n) \cdot n \cdot v \, ds \right) \\
    &+ 2 \int_\Omega u_n \cdot v \, dx - \int_\Omega u_{n-1} \cdot v \, dx \\
    \int_\Omega u_{n+1} \cdot v \, dx &= \frac{\Delta t^2}{\rho} \left( \int_\Omega f \cdot v \, dx - \int_\Omega \lambda(\nabla u_n)(\nabla v) + 2\mu\epsilon(u_n) : \epsilon(v) \, dx + \int_\Gamma g \cdot v \, ds \right) \\
    &+ 2 \int_\Omega u_n \cdot v \, dx - \int_\Omega u_{n-1} \cdot v \, dx
\end{align}
++++

But we can simplify the equation by rewriting the maximal contraction. Let stem:[\epsilon(u_n)] and stem:[\epsilon(v)] be two stem:[3 \times 3] real matrices, with elements stem:[\epsilon(u_n)_{ij}] and stem:[epsilon(v)_{ij}] respectively. The maximal contraction (double dot product) of stem:[\sigma] and stem:[\epsilon] is defined as:
[stem]
++++
\begin{equation}
    \epsilon(u_n) : \epsilon(v) = \sum_{i=1}^{3} \sum_{j=1}^{3} \epsilon(u_n)_{ij} \epsilon(v)_{ij}
\end{equation}
++++
To express this using matrix operations, consider the trace of the product of stem:[\epsilon(u_n)] and stem:[\epsilon(v)^T] (the transpose of stem:[\epsilon(v)]):
[stem]
++++
\begin{equation}
    \text{Tr}(\epsilon(u_n) \epsilon(v)^T) = \sum_{i=1}^{3} (\epsilon(u_n) \epsilon(v)^T)_{ii}
\end{equation}
++++
The element stem:[(\epsilon(u_n) \epsilon(v)^T)_{ii}] is the dot product of the $i$th row of stem:[\epsilon(u_n)] and the $i$th row of $\epsilon(v)^T$. This is equivalent to the sum of the products of corresponding elements in the $i$th row of stem:[\epsilon(u_n)] and the stem:[i]th column of stem:[\epsilon(v)]:
[stem]
++++
\begin{equation}
    (\epsilon(u_n) \epsilon(v)^T)_{ii} = \sum_{j=1}^{3} \epsilon(u_n)_{ij} \epsilon(v)_{ji}
\end{equation}
++++
...
++++
\begin{equation}
    \text{Tr}(\epsilon(u_n) \epsilon(v)^T) = \sum_{i=1}^{3} \sum_{j=1}^{3} \epsilon(u_n)_{ij} \epsilon(v)_{ji}
\end{equation}
++++
Since stem:[\epsilon(v)^T] has stem:[\epsilon(v)_{ji}] as its stem:[(i, j)]-element, the expression becomes:
[stem]
++++
\begin{equation}
    \text{Tr}(\epsilon(u_n) \epsilon(v)^T) = \sum_{i=1}^{3} \sum_{j=1}^{3} \epsilon(u_n)_{ij} \epsilon(v)_{ij} = \epsilon(u_n) : \epsilon(v)
\end{equation}
++++
...
++++
\begin{equation}
    \epsilon(u_n) : \epsilon(v) = \text{Tr}(\epsilon(u_n) \epsilon(v)^T)
\end{equation}
++++
...
++++
\begin{align}
    \int_\Omega u_{n+1} \cdot v \, dx &= \frac{\Delta t^2}{\rho} \left( \int_\Omega f \cdot v \, dx - \int_\Omega \lambda(\nabla u_n)(\nabla v) + 2\mu \text{Tr} \left( \epsilon(u_n) \epsilon(v)^T \right) \, dx + \int_\Gamma g \cdot v \, ds \right) \\
    &+ 2 \int_\Omega u_n \cdot v \, dx - \int_\Omega u_{n-1} \cdot v \, dx
\end{align}
++++